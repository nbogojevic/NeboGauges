...
var $im = {
    ...
    start: function (subscriptions) {
        ...
        $im.gaugesSubscription = subscriptions || $im.gaugesSubscription || {};

        $im.gaugesSocket = new ReconnectingWebSocket("ws://"+location.host+"/_gauges", null, {reconnectInterval: 1000, maxReconnectInterval: 10000});
        $im.gaugesSocket.onopen = function (event) { 
            $('#disconnectedSplash').hide();
            for (var p in $im.gaugesSubscription) {
                $im.gaugesSocket.send("subscribe:"+p);
            }
        };
        $im.gaugesSocket.onmessage = function (event) { 
            var feedData = JSON.parse(event.data);
            var subscription = $im.gaugesSubscription[feedData.feed] || $im.readResult || function (data) { console.log(data); };
            subscription(feedData.data, feedData.feed);
        };
        $im.gaugesSocket.onclose = function (event) {
    ...
    sendEvent: function (event, value)  {
        return $.post($im.apiServer + "/_event/" + event + "/" + (value !== undefined ? value : 0));
    },
